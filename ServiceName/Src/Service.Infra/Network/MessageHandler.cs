...
namespace Service.Infra.Network
{
    public class MessageHandler : DelegatingHandler
    {
        ...
        {
            using (_tracer.BuildSpan(request.RequestUri.LocalPath).StartActive(true))
            {
                ...
                return await _policy.ExecuteAsync((context, token) => base.SendAsync(request, token), new Context(request.RequestUri.PathAndQuery), cancellationToken);
